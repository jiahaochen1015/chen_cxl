...
struct Settings
{
    ...
    uint64_t n_elems;
    uint64_t iter;
    uint64_t prefetchAhead;
    uint64_t batchSize;

    // [New] Runtime Configurable Macros
    uint64_t        pagesPerWarp;       // replaces PAGES_PER_WARP
    uint64_t        nNvmeQueues;        // replaces NUM_NVME_QUEUES
    uint64_t        hostQueueEntries;   // replaces HOST_QUEUE_NUM_ENTRIES
    uint64_t        spinThreshold;      // replaces SPIN_COUNT_THRESHOLD
    Settings();
...
void Settings::parseArguments(int argc, char** argv)
{
    OptionMap parsers = {
        ...
        {'X', OptionPtr(new Option<uint64_t>(prefetchAhead, "number", "prefetch_ahead", "Number of pages to prefetch ahead", "0"))},
        {'Z', OptionPtr(new Option<uint64_t>(batchSize, "number", "batch_size", "NVMe CQ polling batch size (default 32)", "32"))},

        {'Q', OptionPtr(new Option<uint64_t>(nNvmeQueues, "number", "n_nvme_queues", "Number of NVMe Queues to use (Host side)", "4"))},
        {'E', OptionPtr(new Option<uint64_t>(hostQueueEntries, "number", "host_queue_entries", "Number of entries per Host NVMe Queue", "2048"))},
        {'S', OptionPtr(new Option<uint64_t>(spinThreshold, "number", "spin_threshold", "Spin count threshold for prefetch wait", "16384"))},
        {'W', OptionPtr(new Option<uint64_t>(pagesPerWarp, "number", "pages_per_warp", "Number of pages processed per warp", "1"))},
    };
...
Settings::Settings()
{
    ...
    stats = false;
    input = nullptr;
    input_a = "/home/data/datasets/Vectoradd/vector_a.bel";
    input_b = "/home/data/datasets/Vectoradd/vector_b.bel";
    output = nullptr;
    afileoffset = 0;
    bfileoffset = 0;
    //type = 1;
    //memalloc = 2; 
    type = 3; // BASELINE_PC:2, OPTIMIZED_PC:3
    memalloc = 6;
    ...
    pageSize = 1024*64;
    numBase = 1024;
    numElems = (uint64_t)numBase * numBase * numBase * 8;
    repeat = 32;
    src = 0;
    
    maxPageCacheSize = (uint64_t)numBase * numBase * numBase * 8;
