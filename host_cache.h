...
// #define NUM_PREFETCH_THREADS 1w
// // 定义并发*完成* I/O 的线程数量
// #define NUM_COMPLETION_THREADS 1
#define PREFETCH_RING_SIZE (1024 * 256)
...
#if USE_PREFETCH
    GPU_RW_PREFETCH_TO_HOST, // GPU 请求 Host 将页面从 T3 预取到 T2
#endif
    GPU_RW_TYPE_SIZE,
};
...
enum
{
    ...
    HOST_CACHE_ENTRY_PREFETCHING = 2
};
...
// ---------------------------------------------------------------------------
// [New Class] High-Performance SpinLock FreeList
// ---------------------------------------------------------------------------
class SpinLockFreeList
{
private:
    char pad1[64]; // Padding to prevent false sharing
    std::atomic_flag lock = ATOMIC_FLAG_INIT;
    char pad2[64];
    std::vector<uint64_t> slots;

public:
    SpinLockFreeList(uint64_t total_pages)
    {
        slots.reserve(total_pages);
        // Initialize: Fill with all pages (reverse order for intuitive pop)
    ...
    // Try to get a slot (Non-blocking / Spin-wait)
    bool try_get(uint64_t &out_bid)
    {
        if (slots.empty())
...
{
public:
    // Runtime-configurable settings (replaces compile-time macros)
    uint32_t cfg_num_nvme_queues;
    uint32_t cfg_host_queue_entries;
    uint32_t cfg_spin_threshold;

    HostCache(Controller *ctrl, uint64_t gpu_mem_size, uint64_t batch_size,
    ...
    struct alignas(64) PrefetchShard
    {
        std::map<uint16_t, PrefetchCompletionInfo> outstanding_map;
...
#if USE_PREFETCH
    volatile GpuFileRwManager *host_rw_manager_prefetch; // 新增: 预取请求管理器 (Host 指针)
#endif
    volatile HostCacheRuntimeState *hc_runtime_state;
    GpuRwQueue *host_rw_queue;
    GpuRwQueue *host_rw_queue_mem;
#if USE_PREFETCH
    GpuRwQueue *host_rw_queue_prefetch; // 新增: 预取请求队列 (Host 指针)
#endif
    ...
    nvm_queue_host_t *sq_host;
    nvm_queue_host_t *cq_host;
    ...
    PrefetchShard *outstanding_shards;
    std::atomic<bool> completion_thread_running;

    // 结构体整体对齐
    // 确保 sizeof(PrefetchTaskMetadata) 是 64 的倍数，防止数组中相邻 Shard 发生伪共享
    struct alignas(CACHE_LINE_SIZE) PrefetchTaskMetadata
    {
        // 任务数据数组
        // 放在最前面，利用缓存局部性
        uint16_t cid_ring[PREFETCH_RING_SIZE];

        // [Padding] 编译器会自动填充，但为了明确隔离，head 前通常会有空隙

        // 消费者索引 (CompletionLoop 修改)
        // 内部对齐：确保 head 独占 Cache Line，不与 cid_ring 尾部或 tail 冲突
        alignas(CACHE_LINE_SIZE) std::atomic<uint32_t> head{0};

        // 生产者索引 (PrefetchLoop 修改)
        // 内部对齐：确保 tail 独占 Cache Line
        ...
        bool is_full() const
        {
            // [Performance]: 使用位运算 & MASK
            uint32_t next_tail = (tail.load(std::memory_order_relaxed) + 1) & PREFETCH_RING_MASK;
    ...
    {
        PrefetchCompletionInfo requests[PREFETCH_RING_SIZE];
    };
    PerQueueMetadata *qp_metadata;

    PrefetchTaskMetadata *find_prefetch_task;
#endif
    ...
    SpinLockFreeList *free_slots;

    // Smart Allocation Strategy: FreeList (Fast) + Clock Eviction (Slow)
    // Returns true if a slot is found and LOCKED. 'bid' contains the slot index.
    bool allocate_slot_smart(uint64_t &bid)
    {
        // --- Phase 1: FreeList (Fast Path, O(1)) ---
        if (this->free_slots->try_get(bid))
        {

            // Try to acquire the per-slot lock
            if (host_cache_state[bid].lock.exchange(HOST_CACHE_ENTRY_LOCKED, simt::std::memory_order_acquire) == HOST_CACHE_ENTRY_UNLOCKED)
            {
                // Double Check State
                if (host_cache_state[bid].state.load(simt::std::memory_order_relaxed) == HOST_CACHE_ENTRY_INVALID)
                {
                    return true; // Success! Locked and ready.
                }
                // Race condition: State changed unexpectedly. Release lock.
                host_cache_state[bid].lock.store(HOST_CACHE_ENTRY_UNLOCKED, simt::std::memory_order_release);
            }

            // CRITICAL: If we got a bid but failed to use it (race condition), DO NOT put it back immediately
            // to avoid conflict with the thread currently holding it. It will be leaked essentially,
            // but this is extremely rare. Safe over Sorry.
        }

        // --- Phase 2: Clock Eviction (Slow Path, O(N)) ---
        // Scan for VALID + CLEAN + UNPINNED pages to evict.
        // NOTE: Strictly skip INVALID pages here to avoid conflict with FreeList.
        uint64_t start_scan = this->bid.fetch_add(1, simt::memory_order_relaxed) % HOST_MEM_NUM_PAGES;
        uint64_t curr = start_scan;
        int scan_limit = 8192; // Limit scan depth to avoid latency spikes

        for (int i = 0; i < scan_limit; ++i)
        {
            // Check condition: Must be VALID (to evict)
            if (host_cache_state[curr].state.load(simt::std::memory_order_relaxed) == HOST_CACHE_ENTRY_VALID &&
                !host_cache_state[curr].is_dirty &&
                !IS_PINNED(host_cache_state[curr]))
            {
                // Try Lock
                if (host_cache_state[curr].lock.exchange(HOST_CACHE_ENTRY_LOCKED, simt::std::memory_order_acquire) == HOST_CACHE_ENTRY_UNLOCKED)
                {
                    // Double Check
                    if (host_cache_state[curr].state.load(simt::std::memory_order_relaxed) == HOST_CACHE_ENTRY_VALID &&
                        !host_cache_state[curr].is_dirty &&
                    ...
                    {
                        bid = curr;
                        return true; // Found a victim
                    }
                    // Release lock if check fails
                    host_cache_state[curr].lock.store(HOST_CACHE_ENTRY_UNLOCKED, simt::std::memory_order_release);
                }
            }
            curr = (curr + 1) % HOST_MEM_NUM_PAGES;
        }

        return false; // T2 Full / No evictable page found
    }

private:
    uint64_t num_ctrl_pages_in_one_line;
    // 定义分片数量 (必须是 2 的幂，便于位运算)
    static const uint32_t T2_MAP_SHARDS = 256;
    static const uint32_t T2_MAP_SHARD_MASK = T2_MAP_SHARDS - 1;

    // 每个分片一个锁和 Map
    std::mutex t2_map_mutexes[T2_MAP_SHARDS];
    // 映射: 页面全局 Key ( (range_id << 32) | page_idx ) -> T2 槽位 ID (bid)
    std::unordered_map<uint64_t, uint64_t> t2_key_to_bid_maps[T2_MAP_SHARDS];

    // 辅助函数：根据 Key 获取对应的分片
    inline uint32_t get_t2_shard_idx(uint64_t key)
    {
        // 使用 std::hash 进行哈希，然后取模
        return std::hash<uint64_t>{}(key)&T2_MAP_SHARD_MASK;
    }
};
...
#define PRINT_MEM_SAMPLE_STATUS 0
class MemSampleCollector
{
public:
...
size_t pc_mem_size;
static size_t completion_batch_size_g = 32;

// Used to pass configuration from createHostCache() to start_host_runtime_thread()
struct HostCacheLaunchParams
{
    Controller *ctrl;
    size_t pc_mem_size;
    size_t batch_size;
    uint32_t num_nvme_queues;
    uint32_t host_queue_entries;
...
#if USE_PREFETCH
// 预取*提交*线程池
static std::vector<pthread_t> host_prefetch_threads;
// 预取*完成*线程池
static std::vector<pthread_t> host_completion_threads;
#endif
...
// completionLoop 启动函数
static void *start_host_completion_thread(void *arg);
#endif
...
inline uint16_t HostCache::submit_read_data_to_hc_async(const uint64_t starting_lba, const uint64_t n_blocks, const unsigned long long hc_entry, uint32_t queue_id = 0)
{
    nvm_cmd_t cmd;
    //
    uint16_t cid = get_cid(&(this->qp[queue_id]->sq), &(this->sq_host[queue_id]));
    ...
    //
    uint16_t sq_pos = sq_enqueue(&this->qp[queue_id]->sq, &(this->sq_host[queue_id]), &cmd);
...
inline void HostCache::poll_and_complete_io(const uint16_t cid, uint32_t queue_id = 0)
{
    ...
    // 这是唯一的阻塞点
    //
    uint32_t cq_pos = cq_poll(&this->qp[queue_id]->cq, cid, &head, &head_);

    // 执行所有必要的队列清理逻辑
    //
    this->qp[queue_id]->cq.tail.fetch_add(1, simt::memory_order_acq_rel);
    pc_prev_head = this->q_head->load(simt::memory_order_relaxed);
    pc_pos = this->q_tail->fetch_add(1, simt::memory_order_acq_rel);
...
int HostCache::process_queue_batch(uint32_t queue_id, int max_batch)
{
    ...
    int processed = 0;
    uint32_t head = cq->head.load(simt::memory_order_relaxed); // 只读一次 Head
    uint32_t start_head = head;

    uint16_t max_sq_head = 0;
    bool sq_head_updated = false;
    ...
    for (int i = 0; i < max_batch; i++)
    {
        ...
        max_sq_head = (uint16_t)(cpl->dword[2] & 0xffff);
        sq_head_updated = true;

        // [逻辑优化] 这里我们暂不更新 Doorbell，只归还 CID
        // 注意：这里需要一个轻量级的 put_cid，或者直接操作 bitmap
        // 假设 put_cid 开销很小（只是原子变量操作）
        put_cid(sq, sq_h, cid);

        // [处理元数据 - 保持 O(1)]
        uint32_t shard_idx = cid % this->cfg_num_nvme_queues;
        {
    ...
    // --- 2. 批量提交 Doorbell (关键性能点) ---
    if (processed > 0)
    {

        cq->head.store(head, simt::memory_order_release);

        // 更新硬件 Doorbell (MMIO 写)
        uint32_t new_db = head & cq->qs_minus_1;
        *(cq->db) = new_db;

        if (sq_head_updated)
        {
            sq->head.store(max_sq_head, simt::memory_order_release);
        }
    }
...
#if GET_RW_QUEUE_ENTRY_ALTER
    // #if USE_PREFETCH
    //     if (gpu_rw_manager->getRWQueueEntry2_nonblocking(rw_queue_index) == 0)
    //     {
    //         // [死锁避免] 提交失败 (主队列已满)。
    //         // 立即返回新错误码，*不*调用 remote_read_write
    //         return HOST_CACHE_SUBMIT_FAILED;
    //     }
    //     else
    //     {
    //     }
    ...
    which_tier_to_evict(PageReuseTable *prt, uint64_t curr_virt_timestamp, bool &reuse_place_decision, bool dirty = false)
{
    ...
    */
    ...
    // 如果页面只被驱逐过一次（evicted_before 在 update_timestamp_upon_eviction 中累加），历史信息还不足以进行复杂的预测，直接返回上一次重引用时预测的层级
...
#if USE_PREFETCH
    this->host_rw_manager_prefetch->init(this->host_rw_queue_prefetch); // 新增
#endif
...
#if USE_PREFETCH
    this->host_rw_queue_prefetch->init(); // 新增
#endif
...
// Preferred: allocate on specified NUMA node (e.g., CXL node) if enabled
// Define at build time: -DUSE_NUMA_ALLOC and link with -lnuma
#define USE_NUMA_ALLOC 1
#define TARGET_NUMA_NODE 1
    fprintf(stderr, "host_mem allocated  %d  GB.\n", HOST_MEM_SIZE / (1024 * 1024 * 1024));
#if USE_NUMA_ALLOC
    int target_node = TARGET_NUMA_NODE;

    if (numa_available() >= 0)
    {
        host_mem = numa_alloc_onnode((size_t)HOST_MEM_SIZE, target_node);
        if (!host_mem)
        {
            fprintf(stderr, "numa_alloc_onnode failed for node %d\n", target_node);
            exit(1);
        }
        // Touch to commit pages on the target node
        memset(host_mem, 0, (size_t)HOST_MEM_SIZE);
        fprintf(stderr, "host_mem allocated on NUMA node %d .\n", target_node);
        // 使用numa分配host mem会影响到createDma的分配，因此这里直接认为使用了大页，不使用大页的话会导致物理内存不连续而报错
        useHugePage = true;
    }
    else
    {
        // Fallback to original posix_memalign path if NUMA not available
        fprintf(stderr, "libnuma not available, falling back to posix_memalign.\n");
        posix_memalign((void **)&host_mem, PAGE_SIZE, HOST_MEM_SIZE);
        if (!host_mem)
        {
            fprintf(stderr, "posix_memalign failed for host_mem\n");
            exit(1);
        }
        memset(host_mem, 0, (size_t)HOST_MEM_SIZE);
    }
#else
    // Original path: page-aligned + mlock
    posix_memalign((void **)&host_mem, PAGE_SIZE, HOST_MEM_SIZE);
    if (!host_mem)
    {
        fprintf(stderr, "posix_memalign failed for host_mem\n");
    ...
    madvise(host_mem, HOST_MEM_SIZE, MADV_HUGEPAGE);
    ...
    this->host_dma = createDmaOnNode((const nvm_ctrl_t *)_ctrl->ctrl, (size_t)HOST_MEM_SIZE);
    ...
    // Initialize FreeList for fast slot allocation
    this->free_slots = new SpinLockFreeList(HOST_MEM_NUM_PAGES);
    ...
    this->sq_host = new nvm_queue_host_t[this->cfg_num_nvme_queues]();
    this->cq_host = new nvm_queue_host_t[this->cfg_num_nvme_queues]();

#if USE_PREFETCH
    this->outstanding_shards = new PrefetchShard[this->cfg_num_nvme_queues];
#endif

    for (uint32_t i = 0; i < this->cfg_num_nvme_queues; i++)
    {
        this->qp[i] = new QueuePair(_ctrl->ctrl, _ctrl->ns, _ctrl->info, _ctrl->aq_ref, _ctrl->n_qps + i + 1, this->cfg_host_queue_entries);
        this->sq_host[i].cid_h = (padded_struct_h *)malloc(sizeof(padded_struct_h) * 65536);
        memset(this->sq_host[i].cid_h, 0, sizeof(padded_struct_h) * 65536);
        this->cq_host[i].pos_locks_h = (padded_struct_h *)malloc(sizeof(padded_struct_h) * this->cfg_host_queue_entries);
        memset(this->cq_host[i].pos_locks_h, 0, sizeof(padded_struct_h) * this->cfg_host_queue_entries);
    }
#if USE_PREFETCH
    this->find_prefetch_task = new PrefetchTaskMetadata[this->cfg_num_nvme_queues];
    memset(this->find_prefetch_task, 0, sizeof(PrefetchTaskMetadata) * this->cfg_num_nvme_queues);

    this->qp_metadata = new PerQueueMetadata[this->cfg_num_nvme_queues];
    memset(this->qp_metadata, 0, sizeof(PerQueueMetadata) * this->cfg_num_nvme_queues);
#endif
    //
...
#if USE_PREFETCH
    fprintf(stderr, "[DEBUG] HostCache initialized with Batch Size: %lu\n", completion_batch_size);
#endif
    // TODO: Handle Profile...
#if APPLY_PROFILE
    readProfiledLog();
...
// 回收P/C线程逻辑
    ...
    for (uint64_t i = 0; i < host_prefetch_threads.size(); i++)
    {
        int ret = pthread_detach(host_prefetch_threads[i]);
    }

    this->prefetchers_exited.store(true, std::memory_order_release);

    std::cerr << "Stopping Completion threads...\n";
    // Join 所有 Completion 线程
    for (uint64_t i = 0; i < host_completion_threads.size(); i++)
    {
    ...
    if (this->free_slots)
    {
        delete this->free_slots;
        this->free_slots = nullptr;
    }

    cudaHostUnregister(host_mem);
    std::cerr << "Unregistered host_mem.\n";
    // if (munlock(host_mem, (size_t)HOST_MEM_SIZE) != 0) {
    //     fprintf(stderr, "WARN: munlock for host_mem failed... (%s)\n", strerror(errno));
    // }
    // std::cerr << "Unregistered and Unlocked host_mem.\n";
#if USER_SPACE_ZERO_COPY == 0
    if (munlock(host_mem, (size_t)HOST_MEM_SIZE) != 0)
    {
        fprintf(stderr, "WARN: munlock for host_mem failed... (%s)\n", strerror(errno));
...
#if USE_NUMA_ALLOC
    std::cerr << "About to numa_free host_mem...\n";
    numa_free(host_mem, (size_t)HOST_MEM_SIZE);
    std::cerr << "Freed host_mem with numa_free.\n";
#else
    std::cerr << "About to free host_mem...\n";
    free(host_mem);
    std::cerr << "Freed host_mem.\n";
#endif
#else
    if (prp2_dma)
    {
        if (munlock(prp2_dma->vaddr, (size_t)this->num_pages * ctrl->ctrl->page_size) != 0)
        {
            fprintf(stderr, "WARN: munlock for prp2_dma failed... (%s)\n", strerror(errno));
        }
    }

    if (host_dma)
    {
        if (munlock(host_dma->vaddr, (size_t)HOST_MEM_SIZE) != 0)
        {
            fprintf(stderr, "WARN: munlock for host_dma failed... (%s)\n", strerror(errno));
        }
    }
#endif
...
// readProfiledLog();
    ...
    std::cerr << "About to delete NVME queues...\n";
    for (uint32_t i = 0; i < this->cfg_num_nvme_queues; i++)
    {
        std::cerr << "About to delete NVME Queue " << i << "...\n";
        delete this->qp[i];
    ...
    this->qp = nullptr;
    delete[] this->sq_host;
    this->sq_host = nullptr;
    delete[] this->cq_host;
    this->cq_host = nullptr;
#if USE_PREFETCH
    delete[] this->outstanding_shards;
    this->outstanding_shards = nullptr;
    delete[] this->find_prefetch_task;
    this->find_prefetch_task = nullptr;
    delete[] this->qp_metadata;
    this->qp_metadata = nullptr;
#endif
    std::cerr << "Finished deleting NVME queues.\n";

    std::cerr << "About to delete stream_mngr...\n";
    delete stream_mngr;
    std::cerr << "Deleted stream_mngr.\n";
...
///*
    ...
    if (type == GPU_RW_EVICT_TO_HOST)
    {
        ...
        } while ((host_cache_state[_bid].state == HOST_CACHE_ENTRY_VALID && (host_cache_state[_bid].is_dirty || IS_PINNED(host_cache_state[_bid]))) || host_cache_state[_bid].state == HOST_CACHE_ENTRY_PREFETCHING);
...
// Lastly, do data transfer
    ...
    else if (type == GPU_RW_UNPIN_PAGE)
    {
        ...
        if (host_cache_state[_bid].is_dirty)
        {
            ...
            uint32_t old_state = host_cache_state[_bid].state.exchange(HOST_CACHE_ENTRY_INVALID, simt::std::memory_order_release);

#if SMART_SCAN
            if (old_state != HOST_CACHE_ENTRY_INVALID)
            {
#if USE_PREFETCH
                // Remove from Map
                uint64_t key_to_remove = host_cache_state[_bid].tag;
                uint32_t shard = get_t2_shard_idx(key_to_remove);
                {
                    std::lock_guard<std::mutex> lock(t2_map_mutexes[shard]);
                    t2_key_to_bid_maps[shard].erase(key_to_remove);
                }
#endif

                // Unlock BEFORE put (so other threads can pick it up immediately)
                host_cache_state[_bid].lock.store(HOST_CACHE_ENTRY_UNLOCKED, simt::std::memory_order_release);

                // [New] Return to FreeList
                this->free_slots->put(_bid);

                // Skip the standard unlock below since we already did it
                goto unpin_done;
            }
#endif // SMART_SCAN
       // 072223
    ...
    {
#if USE_PREFETCH
        // 忽略 GPU 发来的陈旧 bid，必须使用 Key 和 Map 重新查找。
        bool fail = true;
        uint64_t _bid = 0; // 我们将重新查找它
        ...
        if (found_in_map)
        {
            ...
            do
            {
                ...
                if (host_cache_state[_bid].state.load(simt::memory_order_relaxed) != HOST_CACHE_ENTRY_VALID || host_cache_state[_bid].tag != key)
                {
                    // #if PREFETCH_DEBUG
                    //                     // [日志补丁 1.3: 竞态导致失败]
                    //                     fprintf(stderr, "[%.3fms] Host(Fetch) [Q:%d]: Key 0x%lx FAILED T2 validation (State/Tag mismatch). Aborting fetch.\n",
                    //                             std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(std::chrono::high_resolution_clock::now().time_since_epoch()).count(),
                    //                             queue_index, key);
                    // #endif
                    break;
                }
                        ...
                        if (state == HOST_CACHE_ENTRY_VALID && host_cache_state[_bid].tag == key)
                        {
                            ...
                            if (host_cache_state[_bid].is_dirty)
                            {
                                ...
                                if (old_state != HOST_CACHE_ENTRY_INVALID)
                                {
                                    ...
                                    // Skip standard unlock
                                    goto fetch_unlock_done;
                                }
#endif
        ...
        else
        {
            // #if PREFETCH_DEBUG
            //             // [日志补丁 1.6: T2 未命中]
            //             fprintf(stderr, "[%.3fms] Host(Fetch) [Q:%d]: Key 0x%lx NOT_FOUND in T2 map. Prefetch might have failed or been evicted.\n",
...
void HostCache::prefetchLoop()
{
    ...
    while (!revoke_runtime)
    {
        ...
        queue_index = this->global_prefetch_qid.load(std::memory_order_acquire);

        if (this->host_rw_queue_prefetch->entries[queue_index & (GPU_RW_SIZE - 1)].status == GPU_RW_PENDING)
        {

            bool exchanged = this->global_prefetch_qid.compare_exchange_weak(queue_index, queue_index + 1, std::memory_order_release);
            if (!exchanged)
            {
                continue;
            }

            uint32_t original_queue_index = queue_index & (GPU_RW_SIZE - 1);

            this->host_rw_queue_prefetch->entries[original_queue_index].status = GPU_RW_PROCESSING;
            __sync_synchronize();

            uint64_t key = this->host_rw_queue_prefetch->entries[original_queue_index].key;
            bool already_in_t2 = false;
            // #if PREFETCH_DEBUG
            //             fprintf(stderr, "[Prefetch-Submit] [Q: %u] Received request for key 0x%lx\n", original_queue_index, key);
            // #endif
            ...
            {
                // #if PREFETCH_DEBUG
                //                 fprintf(stderr, "[Prefetch-Submit] [Q: %u] Key 0x%lx SKIPPED (Already in T2)\n", original_queue_index, key);
                // #endif
            ...
            else
            {
                ...
                // TODO:优化寻找槽位的算法
                //  --- 阶段 1: 寻找 INVALID (空) 槽位 ---
                do
                {
                    // 检查 state 是否为 INVALID (快速路径)
                    if (host_cache_state[_bid].state.load(simt::std::memory_order_relaxed) == HOST_CACHE_ENTRY_INVALID)
                    {
                        ...
                        {
                            // 再次确认 state 仍为 INVALID (防止竞态)
                            if (host_cache_state[_bid].state.load(simt::std::memory_order_relaxed) == HOST_CACHE_ENTRY_INVALID)
                            {
                                // 成功！我们拥有这个锁和这个 INVALID 槽位
                                found_slot = true;
                                replaced_invalid_slot = true;
                                // #if PREFETCH_DEBUG
                                //                                 fprintf(stderr, "[Prefetch-Submit] [Q: %u] Pass 1: Found INVALID slot %lu for key 0x%lx.\n", queue_index, _bid, key);
                                // #endif
                            ...
                            {
                                // 竞态：在我们锁定的瞬间，它不再是 INVALID (例如被 T_demand 占用了)。
                                // 释放锁并继续扫描下一个槽位。
                                host_cache_state[_bid].lock.store(HOST_CACHE_ENTRY_UNLOCKED, simt::std::memory_order_release);
                            }
                        }
                ...
                // --- 阶段 2: (如果阶段 1 失败) 寻找 VALID, CLEAN, UNPINNED 槽位 ---
                if (!found_slot)
                {
                    _bid = start_bid;
                    do
                    {
                        ...
                        {
                            found_slot = true;
                            replaced_invalid_slot = false; // 修复 C2 - 记录

                            {
                                ...
                                // #if PREFETCH_DEBUG
                                //                                 fprintf(stderr, "[Prefetch-Submit] [Q: %u] Pass 2: Found CLEAN slot %lu (Evicting old key 0x%lx) for new key 0x%lx.\n",
                                //                                         queue_index, _bid, old_key, key);
                                // #endif
                            }
                            break;
                        }
                ...
                if (found_slot)
                {
                    ...
                    uint32_t queue_id = original_queue_index % this->cfg_num_nvme_queues;
                    // #if PREFETCH_DEBUG
                    //                     fprintf(stderr, "[Prefetch-Submit] [Q: %u] Issuing T3->T2 Read (LBA: %lu, T2_Slot: %lu, NVMeQ: %u)\n",
                    //                             original_queue_index, starting_lba, _bid, queue_id);
                    // #endif
                    host_cache_state[_bid].tag = key; // 提前设置 Tag，以便碰撞检测
                    ...
                    // 调用*异步*提交函数
                    uint16_t cid = submit_read_data_to_hc_async(starting_lba, PAGE_SIZE / 512, _bid, queue_id);
                    // #if PREFETCH_DEBUG
                    //                     // [日志补丁 2.1: 预取提交]
                    //                     auto submit_time = std::chrono::high_resolution_clock::now();
                    //                     fprintf(stderr, "[%.3fms] Host(Prefetch-Submit) [Q:%u]: Issuing T3->T2 for Key 0x%lx to T2 Slot %lu (CID %u).\n",
                    //                             std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(submit_time.time_since_epoch()).count(),
                    //                             original_queue_index, key, _bid, cid);
                    // #endif
                    ...
                    info.queue_index = original_queue_index;
...
void HostCache::prefetchLoop(uint64_t worker_id)
{
    ...
    // 步长 = 线程总数
    const uint32_t stride = this->cfg_num_nvme_queues;
    ...
    while (!revoke_runtime)
    {
        ...
        if (q_entries[queue_index].status == GPU_RW_PENDING)
        {
            ...
            else
            {
                ...
                {
#if SMART_SCAN
                    // Check if we are evicting an existing VALID page (Phase 2)
                    if (host_cache_state[_bid].state.load(simt::memory_order_relaxed) == HOST_CACHE_ENTRY_VALID)
                    {
                        replaced_invalid_slot = false;
                    ...
                    {
                        // It was INVALID (from FreeList)
                        replaced_invalid_slot = true;
                    }

                    // Update Map for new key
                    uint32_t new_shard_idx = get_t2_shard_idx(key);
                    {
                        std::lock_guard<std::mutex> lock(t2_map_mutexes[new_shard_idx]);
                        t2_key_to_bid_maps[new_shard_idx][key] = _bid;
                    ...
                    // 任务记录
                    uint32_t current_tail = this->find_prefetch_task[my_queue_id].tail.load(std::memory_order_acquire);
                    uint32_t next_tail = (current_tail + 1) & PREFETCH_RING_MASK;
                    if (next_tail != this->find_prefetch_task[my_queue_id].head.load(std::memory_order_acquire))
                    {
                        this->find_prefetch_task[my_queue_id].cid_ring[current_tail] = cid;
                        this->find_prefetch_task[my_queue_id].tail.store(next_tail, std::memory_order_release);
                    }
                }
                else
                {
...
void HostCache::completionLoop(uint64_t worker_id)
{
    ...
    // Worker ID 1:1 映射到 NVMe Queue ID
    uint32_t my_queue_id = (uint32_t)worker_id;
    PrefetchTaskMetadata *my_task = &this->find_prefetch_task[my_queue_id];
    PerQueueMetadata *my_meta = &this->qp_metadata[my_queue_id];

    while (this->completion_thread_running.load(std::memory_order_acquire))
    {
        uint16_t cid;
        uint32_t current_head = my_task->head.load(std::memory_order_relaxed);
        if (current_head != my_task->tail.load(std::memory_order_acquire))
        {
            cid = my_task->cid_ring[current_head];
            my_task->head.store((current_head + 1) & PREFETCH_RING_MASK, std::memory_order_release);
        }
        else
        {
            continue;
...
void HostCache::threadLoop(int tid)
{
    ...
    while (1)
    {
        ...
        {
            // fprintf(stderr, "[DEBUG] T2-Worker[%d]: Found PENDING task at Q:%u.\n", tid, (queue_index & (GPU_RW_SIZE - 1)));
            // fflush(stderr);
...
static void *host_flush_page(void *arg)
{
    ...
    uint32_t queue_id = start_page % (host_cache ? host_cache->cfg_num_nvme_queues : 1);
...
static void *start_host_runtime_thread(void *arg)
{
    ...
    HostCacheLaunchParams *params = (HostCacheLaunchParams *)arg;
    host_cache = new HostCache(params->ctrl,
                               params->pc_mem_size,
                               params->batch_size,
                               params->num_nvme_queues,
                               params->host_queue_entries,
                               params->spin_threshold);
    delete params;
    ...
    fprintf(stderr, "[DEBUG] host_runtime_thread: Calling launchThreadLoop().\n"); // 关键日志
...
{
    uint64_t worker_id = (uint64_t)arg;
    printf("Host Prefetch Completion Thread %lu Started...\n", worker_id);
    ...
    int target_core = CORE_OFFSET + (int)worker_id + (int)host_cache->cfg_num_nvme_queues;
    CPU_SET(target_core, &cpuset);
    ...
    if (worker_id == 0)
    {
        host_cache->completion_thread_running.store(true, std::memory_order_release);
    }
    else
    {
                                         ...
                                         uint32_t n_queues, uint32_t q_entries, uint32_t spin_thresh)
{
    ...
    HostCacheLaunchParams *params = new HostCacheLaunchParams();
    params->ctrl = ctrl;
    params->pc_mem_size = _pc_mem_size;
    params->batch_size = _batch_size;
    params->num_nvme_queues = n_queues;
    params->host_queue_entries = q_entries;
    params->spin_threshold = spin_thresh;

    ret = pthread_create(&host_runtime_thread, NULL, &start_host_runtime_thread, (void *)params);
    if (ret != 0)
    {
        handle_error_en(ret, "Create Host Runtime Failed...");
    }

    fprintf(stderr, "[DEBUG] Main Thread: Waiting in 'while (host_cache == NULL)' loop...\n");
    while (host_cache == NULL)
        ;
    ...
    // spin_init();
    // #endif
    fprintf(stderr, "[DEBUG] Main Thread: 'host_cache' pointer is valid. Exiting createHostCache.\n");
...
static inline HostCache *createHostCache(Controller *ctrl, size_t _pc_mem_size, size_t _batch_size)
{
    return createHostCache(ctrl, _pc_mem_size, _batch_size, 4, 2048, 16384);
}

static inline HostCache *createHostCache(Controller *ctrl, size_t _pc_mem_size)
{
    return createHostCache(ctrl, _pc_mem_size, completion_batch_size_g, 4, 2048, 16384);
}

static inline HostCache *createHostCache(Controller *ctrl)
{
    return createHostCache(ctrl, 0, completion_batch_size_g, 4, 2048, 16384);
}
...
{
    int ret;
    uint32_t n_queues = host_cache->cfg_num_nvme_queues;
    fprintf(stderr, "[DEBUG] Main Thread: ENTERED LaunchPrefetchThreads(). Starting %u + %u prefetch/completion threads...\n", n_queues, n_queues);

    host_prefetch_threads.resize(n_queues);
    host_completion_threads.resize(n_queues);
    ...
    for (uint64_t i = 0; i < n_queues; i++)
    {
        ...
        if (ret != 0)
        {
            handle_error_en(ret, "Create Host Prefetch Submit Thread Failed...");
        }
    }
    ...
    for (uint64_t i = 0; i < n_queues; i++)
    {
        ...
        fflush(stderr);
        ret = pthread_create(&host_completion_threads[i], NULL, &start_host_completion_thread, (void *)i);
        if (ret != 0)
        {
            handle_error_en(ret, "Create Host Prefetch Completion Thread Failed...");
        }
    }
    fprintf(stderr, "[DEBUG] Main Thread: EXITED LaunchPrefetchThreads().\n");
}
#endif
void flushHostCache()
{
