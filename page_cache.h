...
// #if USE_HOST_CACHE
// #endif
__device__ volatile unsigned long long gmt_debug_key = 0xDEADBEEF;
__device__ volatile int gmt_debug_qid = -1;
...
#if PREFETCH_DEBUG
    // 用于验证 T1 Miss 路径的原子计数器
    simt::atomic<uint64_t, simt::thread_scope_device> *t1_miss_t2_hit_count;
    simt::atomic<uint64_t, simt::thread_scope_device> *t1_miss_t2_miss_count;
    simt::atomic<uint64_t, simt::thread_scope_device> *t1_miss_t3_fetch_count;

    // 用于验证 Prefetch 命中路径的原子计数器
    simt::atomic<uint64_t, simt::thread_scope_device> *prefetch_hit_count;
    simt::atomic<uint64_t, simt::thread_scope_device> *prefetch_miss_stale_count;

    // 用于验证 T1 Evict 路径的原子计数器
    simt::atomic<uint64_t, simt::thread_scope_device> *t1_evict_t2_insert_count;
    simt::atomic<uint64_t, simt::thread_scope_device> *t1_evict_t3_discard_count;

    // [探针 H3] find_slot 活锁检测
    simt::atomic<uint64_t, simt::thread_scope_device> *find_slot_total_scans;
    simt::atomic<uint64_t, simt::thread_scope_device> *find_slot_skipped_prefetch;

    // [探针 H2] find_page 活锁检测
    simt::atomic<uint64_t, simt::thread_scope_device> *find_page_submit_failed_retries;
#endif
...
struct page_cache_t
{
    ...
    // 它包含了所有需要被 GPU 核函数直接高速访问和修改的页缓存元数据和状态信息，后续会被copy到GPU上，其地址存在page_cache_d_t* d_pc_ptr
    page_cache_d_t pdt;

    // void* d_pc;
    // BufferPtr prp2_list_buf;
    // bool prps;
    // 是一个主机端的指针数组，其中每个元素是一个指向特定逻辑数据范围 (range_t) 的所有页的元数据 (data_page_t 数组) 的设备指针，同样会被copy到GPU上
    pages_t *h_ranges;
    page_reuse_hist_t **h_ranges_reuse_hist;

    uint64_t *h_ranges_page_starts;
    data_dist_t *h_ranges_dists;
    // 在gpu上的pdt实例，给gpu操作缓存的接口，GPU 核函数通过这个 d_pc_ptr 来访问和操作页缓存的设备端状态和元数据。
    // 例如，在 range_d_t 的方法中，cache.base_addr 实际上是 d_pc_ptr->base_addr。
...
#if PREFETCH_DEBUG
    BufferPtr t1_miss_t2_hit_buf;
    BufferPtr t1_miss_t2_miss_buf;
    BufferPtr t1_miss_t3_fetch_buf;
    BufferPtr prefetch_hit_buf;
    BufferPtr prefetch_miss_stale_buf;
    BufferPtr t1_evict_t2_insert_buf;
    BufferPtr t1_evict_t3_discard_buf;
#endif
    ...
    void print_reset_stats(void)
    {
        ...
        uint64_t t1_miss_t2_hit = 0;
        uint64_t t1_miss_t2_miss = 0;
        uint64_t t1_miss_t3_fetch = 0;
        uint64_t prefetch_hit = 0;
        uint64_t prefetch_miss_stale = 0;
        uint64_t t1_evict_t2_insert = 0;
        uint64_t t1_evict_t3_discard = 0;

        cuda_err_chk(cudaMemcpy(&t1_miss_t2_hit, pdt.t1_miss_t2_hit_count, sizeof(uint64_t), cudaMemcpyDeviceToHost));
        cuda_err_chk(cudaMemcpy(&t1_miss_t2_miss, pdt.t1_miss_t2_miss_count, sizeof(uint64_t), cudaMemcpyDeviceToHost));
        cuda_err_chk(cudaMemcpy(&t1_miss_t3_fetch, pdt.t1_miss_t3_fetch_count, sizeof(uint64_t), cudaMemcpyDeviceToHost));
        cuda_err_chk(cudaMemcpy(&prefetch_hit, pdt.prefetch_hit_count, sizeof(uint64_t), cudaMemcpyDeviceToHost));
        cuda_err_chk(cudaMemcpy(&prefetch_miss_stale, pdt.prefetch_miss_stale_count, sizeof(uint64_t), cudaMemcpyDeviceToHost));
        cuda_err_chk(cudaMemcpy(&t1_evict_t2_insert, pdt.t1_evict_t2_insert_count, sizeof(uint64_t), cudaMemcpyDeviceToHost));
        cuda_err_chk(cudaMemcpy(&t1_evict_t3_discard, pdt.t1_evict_t3_discard_count, sizeof(uint64_t), cudaMemcpyDeviceToHost));

        std::cout << "*********************************" << std::endl;
        std::cout << "****** GMT T1/T2 INTERNALS ******" << std::endl;
        std::cout << std::dec << "T1 Miss -> T2 Hit (Demand):   " << t1_miss_t2_hit << std::endl;
        std::cout << std::dec << "T1 Miss -> T2 Miss (Demand):  " << t1_miss_t2_miss << std::endl;
        std::cout << std::dec << "T1 Miss -> T3 Fetch (Demand): " << t1_miss_t3_fetch << std::endl;
        std::cout << std::dec << "T1 Miss -> T2 Hit (Prefetch): " << prefetch_hit << std::endl;
        std::cout << std::dec << "T1 Miss -> T2 Miss (Stale PF):" << prefetch_miss_stale << std::endl;
        std::cout << std::dec << "T1 Evict -> T2 Insert (Clean):" << t1_evict_t2_insert << std::endl;
        std::cout << std::dec << "T1 Evict -> T3 Discard (Clean):" << t1_evict_t3_discard << std::endl;
        std::cout << "*********************************" << std::endl;
#endif
...
#if PREFETCH_DEBUG
        // 分配并清零 T1 Miss 计数器
        t1_miss_t2_hit_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        t1_miss_t2_miss_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        t1_miss_t3_fetch_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        pdt.t1_miss_t2_hit_count = (simt::atomic<uint64_t, simt::thread_scope_device> *)t1_miss_t2_hit_buf.get();
        pdt.t1_miss_t2_miss_count = (simt::atomic<uint64_t, simt::thread_scope_device> *)t1_miss_t2_miss_buf.get();
        pdt.t1_miss_t3_fetch_count = (simt::atomic<uint64_t, simt::thread_scope_device> *)t1_miss_t3_fetch_buf.get();
        cuda_err_chk(cudaMemset(pdt.t1_miss_t2_hit_count, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));
        cuda_err_chk(cudaMemset(pdt.t1_miss_t2_miss_count, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));
        cuda_err_chk(cudaMemset(pdt.t1_miss_t3_fetch_count, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));

        // 分配并清零 Prefetch 命中计数器
        prefetch_hit_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        prefetch_miss_stale_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        pdt.prefetch_hit_count = (simt::atomic<uint64_t, simt::thread_scope_device> *)prefetch_hit_buf.get();
        pdt.prefetch_miss_stale_count = (simt::atomic<uint64_t, simt::thread_scope_device> *)prefetch_miss_stale_buf.get();
        cuda_err_chk(cudaMemset(pdt.prefetch_hit_count, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));
        cuda_err_chk(cudaMemset(pdt.prefetch_miss_stale_count, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));

        // 分配并清零 T1 Evict 计数器
        t1_evict_t2_insert_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        t1_evict_t3_discard_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        pdt.t1_evict_t2_insert_count = (simt::atomic<uint64_t, simt::thread_scope_device> *)t1_evict_t2_insert_buf.get();
        pdt.t1_evict_t3_discard_count = (simt::atomic<uint64_t, simt::thread_scope_device> *)t1_evict_t3_discard_buf.get();
        cuda_err_chk(cudaMemset(pdt.t1_evict_t2_insert_count, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));
        cuda_err_chk(cudaMemset(pdt.t1_evict_t3_discard_count, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));

        // [探针 H3] 分配
        BufferPtr find_slot_total_scans_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        BufferPtr find_slot_skipped_prefetch_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        pdt.find_slot_total_scans = (simt::atomic<uint64_t, simt::thread_scope_device> *)find_slot_total_scans_buf.get();
        pdt.find_slot_skipped_prefetch = (simt::atomic<uint64_t, simt::thread_scope_device> *)find_slot_skipped_prefetch_buf.get();
        cuda_err_chk(cudaMemset(pdt.find_slot_total_scans, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));
        cuda_err_chk(cudaMemset(pdt.find_slot_skipped_prefetch, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));

        // [探针 H2] 分配
        BufferPtr find_page_submit_failed_retries_buf = createBuffer(sizeof(simt::atomic<uint64_t, simt::thread_scope_device>), cudaDevice);
        pdt.find_page_submit_failed_retries = (simt::atomic<uint64_t, simt::thread_scope_device> *)find_page_submit_failed_retries_buf.get();
        cuda_err_chk(cudaMemset(pdt.find_page_submit_failed_retries, 0, sizeof(simt::atomic<uint64_t, simt::thread_scope_device>)));
#endif
...
struct range_d_t
{
    ...
    __forceinline__
        __device__ void
        prefetch_page_to_t2_async(uint64_t prefetch_page_idx);
#endif
};
...
// Timestamp...
    ...
    do
    {
        ...
        case NV_NB:
        {
            st_new = pages[index].state.load(simt::memory_order_relaxed);
#if USE_PREFETCH
            // [PATH 1: PREFETCH ENABLED]
            // 1a. 检查是否处于预取中间态
            if ((st_new & UNDER_PREFETCH) != 0)
            {
                // [日志补丁: GPU 侧确认碰撞]
                // (注意：printf 在 CUDA 中是序列化的，可能有性能影响，但对调试至关重要)
                // #if PREFETCH_DEBUG
                //                 printf("GPU [TID: %u, Key: 0x%llx]: Hit UNDER_PREFETCH. Issuing T2->T1 fetch RPC.\n",
                //                        blockIdx.x * blockDim.x + threadIdx.x,
                //                        (unsigned long long)(((uint64_t)this->range_id << 32) | index));
                // #endif

                // 尝试获取 BUSY 锁来 "完成" 这个预取
                st_new = pages[index].state.fetch_or(BUSY, simt::memory_order_acquire);

                if ((st_new & BUSY) != 0)
                {
                    // 没抢到锁，其他线程正在处理。自旋。
                    fail = true; // 触发外层 do-while 循环
                    break;       // 退出 switch
                }

                // [+++++ 补丁 1：打破预取路径的死锁 +++++]
                // 释放 Lock B (BUSY)，因为我们即将调用 find_slot (它会获取 Lock A)
                pages[index].state.fetch_and(DISABLE_BUSY_MASK, simt::memory_order_release);

                // [成功抢到锁]
                // 尝试从 T2 (Host Cache) 抓取
                // (2) 现在安全地调用 find_slot()
                uint32_t page_trans = cache.find_slot(index, range_id, queue);

                // (3) 重新获取 Lock B (BUSY)，为 I/O 做准备
                unsigned int re_ns_pf = 8;
                while (true)
                {
                    st_new = pages[index].state.fetch_or(BUSY, simt::memory_order_acquire);
                    if ((st_new & BUSY) == 0)
                    ...
                    // 失败了（竞态），释放我们失败的锁尝试，然后自旋
                    pages[index].state.fetch_and(DISABLE_BUSY_MASK, simt::memory_order_release);
                    __nanosleep(re_ns_pf);
                    if (re_ns_pf < 256)
                        re_ns_pf *= 2;
                }

                // uint32_t page_trans = cache.find_slot(index, range_id, queue);
                bool is_cached_page_dirty = false; // T2 抓取时需要
                int hc_ret = accessHostCache((void *)get_cache_page_addr(page_trans),
                                             ((range_id << 32) | index) /*key*/,
                                             GPU_RW_FETCH_FROM_HOST,
                                             &(pages[index].bid), false, &is_cached_page_dirty);

//                 if (hc_ret == HOST_CACHE_SUBMIT_FAILED)
//                 {
// #if PREFETCH_DEBUG
//                     // [探针 H2] 记录这次失败的重试
//                     cache.find_page_submit_failed_retries->fetch_add(1, simt::memory_order_relaxed);
// #endif
//                     // [死锁 B 避免] 提交失败! 主队列已满。
//                     // 释放 T1 BUSY 锁并强制外部循环重试。
//                     pages[index].state.fetch_and(DISABLE_BUSY_MASK, simt::memory_order_release);
//                     fail = true; // 重试
//                     break;       // 退出 switch
//                 }
                ...
                {
                    cache.prefetch_hit_count->fetch_add(1, simt::memory_order_relaxed);
                }
                else
                {
                    cache.prefetch_miss_stale_count->fetch_add(1, simt::memory_order_relaxed);
                }
#endif
                if (hc_ret == HOST_CACHE_SUCCEEDED)
                {
                    ...
                    // 解决竞态问题
                    uint32_t final_state = (st_new & ~(UNDER_PREFETCH)) | (VALID | CACHED_BY_HOST);

                    // 如果 T2 是脏的，或者这是一个写操作，T1 也必须是脏的
                    if (write || is_cached_page_dirty)
                        final_state |= DIRTY;

                    // 用一次 'store' 原子地释放 BUSY 锁并更新状态
                    pages[index].state.store(final_state, simt::memory_order_release);

                    // if (write || is_cached_page_dirty) // 如果T2页面是脏的，T1也必须标记为脏
                    //     pages[index].state.fetch_or(DIRTY, simt::memory_order_relaxed);

                    // // 原子地: 清除 BUSY, 清除 UNDER_PREFETCH, 设置 VALID, 设置 CACHED_BY_HOST
                    // pages[index].state.fetch_xor(BUSY | UNDER_PREFETCH | VALID | CACHED_BY_HOST,
                ...
                {
                    // [失败路径] 预取被丢弃了。
                    // 我们 (按需抓取线程) 必须回退到 T3 (SSD)。
                    //  (清除 UNDER_PREFETCH 标志, 否则 fetch_xor 会失败)
                    // ... (执行 T3 SSD 读取) ...

                    // pages[index].state.fetch_and(~UNDER_PREFETCH, simt::memory_order_relaxed); // 1. 清除标志

                    uint64_t ctrl = get_backing_ctrl(index); // 2. 准备 T3 读取
                    if (ctrl == ALL_CTRLS)
                    ...
                    read_data(&cache, (c->d_qps) + queue, ((b_page)*cache.n_blocks_per_page), cache.n_blocks_per_page, page_trans); // 3. 执行 T3 读取
                    read_io_cnt.fetch_add(1, simt::memory_order_relaxed);

                    pages[index].offset = page_trans; // 4. 设置元数据
                    miss_cnt.fetch_add(count, simt::memory_order_relaxed);

                    // if (write)
                    //     pages[index].state.fetch_or(DIRTY, simt::memory_order_relaxed); // 5. 设置 Dirty

                    uint32_t final_state = (st_new & ~(UNDER_PREFETCH)) | VALID;

                    // 5. 如果是写操作，添加 DIRTY
                    if (write)
                        final_state |= DIRTY;

                    // 6. 释放 BUSY 锁并设置 VALID
                    pages[index].state.store(final_state, simt::memory_order_release);
            ...
            {
#if USE_PREFETCH
                // 再次检查 (Double-Check)，防止竞态
                if ((st_new & UNDER_PREFETCH) != 0)
                {
                    // 竞态：在我们检查 (1a) 和 fetch_or (1b) 之间，它刚被标记为 UNDER_PREFETCH
                    // 释放锁并重试
                    pages[index].state.fetch_and(~BUSY, simt::memory_order_release);
                    fail = true;
                    break; // 退出 switch
                }
#endif
                ...
                if ((st_new & CACHED_BY_HOST) != 0x0)
                {
                    ...
                    {
                        cache.t1_miss_t2_hit_count->fetch_add(1, simt::memory_order_relaxed);
                    }
                    else
                    {
                        cache.t1_miss_t2_miss_count->fetch_add(1, simt::memory_order_relaxed);
                    }
#endif
                    if (ret != HOST_CACHE_SUCCEEDED)
                    {
                ...
                {
#if PREFETCH_DEBUG
                    cache.t1_miss_t3_fetch_count->fetch_add(1, simt::memory_order_relaxed);
#endif
#endif
        ...
        if (fail)
        {
            if (retry_count > 1000000 && (threadIdx.x == 0 && blockIdx.x == 0))
            {
                uint32_t current_state = pages[index].state.load(simt::memory_order_relaxed);
                printf("\n[DEADLOCK TRAP] TID:0 Block:0\n");
                printf("  - Page Index: %lu\n", index);
                printf("  - Raw State : 0x%x\n", current_state);

                // 自动解码状态位（根据 header 定义）
                bool is_valid = current_state & 0x80000000; // VALID_ 0x80000000
                bool is_busy = current_state & 0x40000000;  // BUSY_  0x40000000
                bool is_dirty = current_state & 0x20000000; // DIRTY  0x20000000
                // 假设 UNDER_PREFETCH 是 0x04000000 (请根据 define 确认)
                bool is_pf = current_state & 0x04000000;

                printf("  - Flags: VALID=%d, BUSY=%d, DIRTY=%d, UNDER_PREFETCH=%d\n",
                       is_valid, is_busy, is_dirty, is_pf);
            }
...
//                 }
                ...
                else
                {
                    ...
                    int32_t final_state = (st_new & ~(UNDER_PREFETCH)) | VALID;
...
template <typename T>
__forceinline__
    __device__ void range_d_t<T>::prefetch_page_to_t2_async(uint64_t prefetch_page_idx)
{
    // 1. 尝试原子地将页面从 INVALID 标记为 UNDER_PREFETCH
    // 这是为了确保只有一个线程（在所有 SM 中）会为这个页面提交预取请求
    uint32_t expected_invalid = INVALID;
    uint32_t desired_state = UNDER_PREFETCH;

    if (!pages[prefetch_page_idx].state.compare_exchange_strong(
            expected_invalid, // 期望状态
            desired_state,    // 期望设置的状态
            simt::memory_order_acq_rel,
            simt::memory_order_relaxed))
    {
        // CAS 失败: 页面不是 INVALID。
        // 意味着它已经是 VALID, BUSY, 或者已被其他线程标记为 UNDER_PREFETCH。
        // 无论哪种情况，都无需我们再次预取。
        return;
    }

    // 2. CAS 成功: 我们独占了此页面的预取权。
    //    现在向 Host 端的 *预取* 队列提交请求 (注意 _prefetch 后缀)

    // 2. CAS 成功: 我们独占了预取权。现在尝试非阻塞提交。
    int rw_queue_index = -1;

    // [FIX] 使用新的非阻塞 getRWQueueEntry2_nonblocking
    if (gpu_rw_manager_prefetch->getRWQueueEntry2_nonblocking(rw_queue_index) == 0) // 0 表示失败
    {
        // [FIX] 提交失败 (预取队列已满)!
        // 我们必须回滚状态。
        pages[prefetch_page_idx].state.store(INVALID, simt::memory_order_release);
        return; // 放弃此次预取
    }

    // 准备请求负载 (使用 LocalEntry 以优化 memcpy)
    LocalEntry local;
    local.type = GPU_RW_PREFETCH_TO_HOST;
    local.key = ((uint64_t)this->range_id << 32) | prefetch_page_idx; // 构造全局 Key
    local.gpu_addr = nullptr;                                         // 预取 T3->T2，不需要 GPU 地址
    local.dirty = false;
    local.bid = -1; // BID (T2 槽位) 将由 Host 端分配

    // 复制请求到共享队列
    memcpy((void *)&(gpu_rw_queue_prefetch->entries[rw_queue_index]), (void *)&local, sizeof(LocalEntry));
    __threadfence_system(); // 确保 memcpy 对 Host 可见

    // if (rw_queue_index < 100)
    // { // 限制输出防止刷屏
    //     printf("[GPU_PROBE] T1 Prefetch Submitted: QID=%d | Key=0x%llx\n",
    //            rw_queue_index, (unsigned long long)local.key);
    // }
    // 3. 设置 PENDING，通知 Host 开始工作
    gpu_rw_queue_prefetch->entries[rw_queue_index].status = GPU_RW_PENDING;
    __threadfence_system(); // 确保状态更新对 Host 可见

    // 4. "即发即忘" (Fire-and-Forget)
    //    我们 *不* 在这里调用 putRWQueueEntry。
    //    正如第 3 部分所实现的，Host 端的 prefetchLoop 会在处理完请求后
    //    通过设置 entry_lock = GPU_NO_LOCKED 来释放这个槽位。
}
#endif // USE_PREFETCH
...
// #endif
...
#if USE_PREFETCH
    // --- 新增: 手动预取辅助函数 ---
    // 这个函数是 "即发即忘" 的，设计为由 Warp Leader 调用
    __forceinline__
        __device__ void
        prefetch_page_by_index(const size_t element_index) const
    {
        // 1. 找到此元素索引属于哪个 range
        int64_t r = find_range(element_index); //
        if (r != -1)
        {
            auto r_ = d_ranges + r; //

            // 2. 计算页面 ID
            uint64_t page_idx = r_->get_page(element_index); //

            // 3. 调用我们在第 4 部分创建的预取函数
            //    这会原子性地将页面标记为 CACHED_BY_HOST
            //    并向 host_rw_queue_prefetch 提交一个请求
            r_->prefetch_page_to_t2_async(page_idx); //
        }
    }
    // --- 结束 ---
#endif // USE_PREFETCH
            ...
            T seq_read(const size_t i) const
    {
        ...
        int64_t r = find_range(i);
        auto r_ = d_ranges + r;
        int64_t my_page_idx = -1;

        if (r != -1)
        {
            my_page_idx = r_->get_page(i);
        }
        ...
        page_cache_d_t::find_slot(uint64_t address, uint64_t range_id, const uint32_t queue_, int32_t *bid)
{
    ...
    do
    {
        ...
        bool lock = false;
#if PREFETCH_DEBUG
        this->find_slot_total_scans->fetch_add(1, simt::memory_order_relaxed);
#endif
        /*拿到这个物理槽位的状态，分为三种情况：
        1. free：证明这个槽位处于空闲状态，其上的page被已经被驱逐或者是还没使用过。
        2. unlocked：表示这个槽位有内容，但是没有线程在使用这个槽位。
...
// update_timestamp_upon_re_reference(&(ranges_reuse_hist[range_id][address]), this->virt_timestamp->load(simt::memory_order_relaxed));
...
#if USE_PREFETCH
                // [PATH 1: PREFETCH ENABLED]
                // 检查页面是否空闲 (cnt=0) 或仅被预取占用 (cnt=UNDER_PREFETCH)
                // if ((b == 0) && ((cnt == 0) || (cnt == UNDER_PREFETCH)))

#if PREFETCH_DEBUG
                // [探针 H3] 检测是否因 UNDER_PREFETCH 而跳过
                if ((b == 0) && (cnt == UNDER_PREFETCH))
                {
                    this->find_slot_skipped_prefetch->fetch_add(1, simt::memory_order_relaxed);
                }
#endif
                if ((b == 0) && (cnt == 0))
                {
                    ...
                    // 再次检查 (Double-Check)
                    uint32_t new_cnt = new_expected_state & CNT_MASK;

                    // 必须同时检查：1. 我们成功获取了 BUSY 锁
                    //             2. 并且状态在我们获取锁后仍然安全

                    if (((new_expected_state & BUSY) == 0) && // 1. 成功抢到 BUSY 锁
                        (new_cnt == 0))                       // 2. 状态仍然安全
                    {
...
#endif
                                        ranges_reuse_hist[previous_range][previous_address].evict_attempt_num++;
                                        goto unlock_page; // [!!] 跳转到下方的 'unlock_page' 标签
                                    }
                                    else
                                    {
#endif // END TMM
                        ...
                        else
                        {
                            ...
                            if (tier_to_evict == Tier2_CPU && !idle_slots_too_few(64))
                            {
                                if ((expected_state & CACHED_BY_HOST) == 0x0)
                                {
                                    ...
                                    if (ret == HOST_CACHE_SUCCEEDED)
                                    {
                                        this->t1_evict_t2_insert_count->fetch_add(1, simt::memory_order_relaxed);
                                    }
#endif
                                    if (ret == HOST_CACHE_SUCCEEDED)
                                    {
                            ...
                            {
#if PREFETCH_DEBUG
                                this->t1_evict_t3_discard_count->fetch_add(1, simt::memory_order_relaxed);
#endif
#if PRINT_TMM
                                printf("[%u][%u] discarded (clean)\n", previous_range, previous_address);
#endif
      ...
      // (省略调试代码)
                        ...
                        // [!! (状态清除) !!]
                        // 检查我们是否要保留 CACHED_BY_HOST 标志 (如果驱逐到T2)
                        uint32_t flags_to_keep = (this->ranges[previous_range][previous_address].state.load(simt::memory_order_relaxed) & CACHED_BY_HOST);

                        // 原子地设置状态为 INVALID (0) | flags_to_keep
                        // 这将清除 BUSY, DIRTY, VALID, 以及卡住的 UNDER_PREFETCH
                        this->ranges[previous_range][previous_address].state.store(
                            INVALID | flags_to_keep,
                            simt::memory_order_release);
...
// (省略调试代码)
                    ...
                    {
                        // [!! (修复死锁) !!]
                        // 抢锁失败 (new_expected_state & BUSY != 0)，
                        // 或者状态已改变 (new_cnt != 0 && new_cnt != UNDER_PREFETCH)

                    unlock_page:
                        // 只有当我们之前成功获取了 BUSY 锁时，才需要释放它
                        if ((new_expected_state & BUSY) == 0)
                        {
                            this->ranges[previous_range][previous_address].state.fetch_and(DISABLE_BUSY_MASK, simt::memory_order_release);
                        }
                        // (如果 (new_expected_state & BUSY) != 0，说明我们没抢到锁，无需操作)
                    }
                }
#else // (USE_PREFETCH == 0)
      // [PATH 2: PREFETCH DISABLED]

                if ((cnt == 0) && (b == 0)) // 原始检查
                {
                    new_expected_state = this->ranges[previous_range][previous_address].state.fetch_or(BUSY, simt::memory_order_acquire);
                    uint32_t new_cnt = new_expected_state & CNT_MASK; // 重新检查 cnt

                    if (((new_expected_state & BUSY) == 0)) // 成功抢到 BUSY 锁
                    {
                        if (new_cnt == 0) // 状态仍然安全 (原始检查)
                        {
...
//__threadfence();
                        ...
                        {
                            // [!! (修复死锁) !!]
                            // 抢到了 BUSY 锁，但状态变了 (new_cnt != 0)，必须释放锁
                            goto unlock_page;
                        }
                    unlock_page:
                        this->ranges[previous_range][previous_address].state.fetch_and(DISABLE_BUSY_MASK, simt::memory_order_release);
